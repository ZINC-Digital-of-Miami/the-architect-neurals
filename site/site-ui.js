...
(() => {
  ...
  window.addEventListener('beforeprint', () => {
    detailsOpenedForPrint = [...document.querySelectorAll('details:not([open])')];
    detailsOpenedForPrint.forEach(details => { details.open = true; });
  });
  window.addEventListener('afterprint', () => {
    detailsOpenedForPrint.forEach(details => { details.open = false; });
    detailsOpenedForPrint = [];
  });
  document.addEventListener('click', event => {
    if (event.target.closest('[data-ui-print]')) window.print();
  });
