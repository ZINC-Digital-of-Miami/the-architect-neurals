# ARCHIVE — src/WEEKLY_RUN.md v2 as of 584f2a2 (sha256 81b12e2bc2acde97…)

Owner ruling 2026-08-22, applied to every cluster C01–C15: REWRITE — "Kill your drift. Your agent instructions are weak as fuck. There is no structure. Get this fixed, make sure the front end does not become a dumpster fire over the coming weeks. Make sure the voice, tone, storytelling and flow are smooth, academic yet very human. Then, only then, start this pig back up." Replaced by v3 in the same commit. The full v2 text follows, byte-for-byte.
...
**Provenance.** Part A below is carried from the original WEEKLY_RUN.md (Google Doc 15bguyjbq2gxwwowpBglyOhLnzIDtBCzUdt2VHOXe7Iw, retrieved 2026-08-22). The A.1 preamble is verbatim. The A.2 sweep blocks were updated on 2026-08-22 for the Consolidated Edition published that day — every change is a dated standing-anchor correction, marked **[upd 08-22]**, never a change to the sweep's scope or discipline; the original wording is recoverable from the Doc. Tense fixes: the original was written as if the Aug 23 run had already passed mid-September dates. Part B was revised on 2026-08-22 to match the v3.2 package: hash-verified pull (`pull_src.sh`), `node src/build_neural_map.js` as the map step, `check.sh` as the guard, `deploy.sh --no-build --prod`, git push as the backup of record, and the zip produced only when the site could not be verified live. Each revision is marked **[v2]** with the reason.
